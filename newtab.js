...
var SYSTEM_THEME_LIGHT = 'Apple';
var SYSTEM_THEME_DARK = 'Midnight';
var SYSTEM_THEME_LIGHT_OVERRIDES = {
	font_size: 15,
	font_color: '#707070'
};
...
function applySelectedTheme(themeName) {
	...
	if (themeName === SYSTEM_THEME_NAME) {
		var overrides = resolvedThemeName === SYSTEM_THEME_DARK ?
			SYSTEM_THEME_DARK_OVERRIDES :
			SYSTEM_THEME_LIGHT_OVERRIDES;
		var mergedTheme = {};
		for (var key in theme)
			mergedTheme[key] = theme[key];
		for (var overrideKey in overrides)
			mergedTheme[overrideKey] = overrides[overrideKey];
		theme = mergedTheme;
	}
}
	...
	switch(key) {
		...
		case 'font_size':
			return '#main a { font-size: ' + (value / 10) + 'em; } ' +
				'#main a.folder, #main a.open { font-size: ' + (value / 10 * 1.04) + 'em; }';
		case 'font_weight':
			return '#main a { font-weight: ' + value + '; }';
		case 'font_color':
			return '#main a { color: ' + value + '; } ' +
				'#main a.folder, #main a.open { color: #1d1d1f; }';
...
function onChange(key, value) {
	...
	if (value != config[key] || key == 'font_size' || key == 'font_color' || key == 'folder_icon_shape' || key == 'v_margin') {
	...
	if (input) {
		...
		if (input.swatch)
			input.swatch.value = isHexColor(value) ? value : config[key];
	}
}
...
function initConfig(key) {
	...
	if (input.type == 'color') {
		...
		swatch.type = 'color';
		var colorValue = getConfig(key);
		swatch.value = isHexColor(colorValue) ? colorValue : config[key];
